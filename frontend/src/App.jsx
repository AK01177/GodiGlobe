...
export default function App() {
  ...
  const onPolClick = cb(p => set({ selC: p.properties.ADMIN, selS: null }), [])
  const onPtClick = cb(x => set({ selS: x.type === 's' ? x.name : null, selC: x.type !== 's' ? x.name : d.selC }), [d.selC])
  const pCap = cb(() => 'rgba(200,200,200,0.02)', [])
  const pSide = cb(() => 'transparent', [])
  const pStr = cb(() => 'rgba(255,255,255,0.25)', [])
  const pCol = cb(() => 'rgba(255,255,255,0.08)', [])
  const ptCol = cb(x => x.type === 's' ? (x.sel ? '#fbbf24' : 'rgba(251,191,36,0.55)') : (x.type === 'cs' ? '#60a5fa' : 'rgba(255,255,255,0.35)'), [])
  const ptRad = cb(x => x.type === 's' ? (x.sel ? 0.35 : 0.15) : (x.type === 'cs' ? 0.4 : 0.2), [])

  return (
    <div className="relative w-screen h-screen bg-black text-white flex overflow-hidden">
      <div className="absolute inset-0 z-0"><Globe globeImageUrl="//unpkg.com/three-globe/example/img/earth-dark.jpg" backgroundImageUrl="//unpkg.com/three-globe/example/img/night-sky.png" polygonsData={d.glPol} polygonCapColor={pCap} polygonSideColor={pSide} polygonStrokeColor={pStr} pathsData={d.glLn} pathPoints={x => x} pathPointLat={p => p.lat} pathPointLng={p => p.lng} pathColor={pCol} pointsData={pts} pointLat="lat" pointLng="lng" pointColor={ptCol} pointRadius={ptRad} pointAltitude={0.01} onPolygonClick={onPolClick} onPointClick={onPtClick} labelsData={lbls} labelLat="lat" labelLng="lng" labelText="name" labelSize={x => x.sz || 0.5} labelDotRadius={0} labelColor={x => x.clr} labelAltitude={0.015} /></div>
